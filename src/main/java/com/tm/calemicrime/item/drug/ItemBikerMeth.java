...
public class ItemBikerMeth extends ItemDrug {

    @Override
    public int getDuration() {
        return CCConfig.drugs.bikerMethEffectDuration.get() * 20;
    }

    @Override
    public void onConsumed(Player player, int additiveDuration) {
        player.addEffect(new MobEffectInstance(InitMobEffects.BIKER_METH_HIGH.get(), getDuration() + additiveDuration));
        player.addEffect(new MobEffectInstance(MobEffects.DIG_SPEED,  getDuration() + additiveDuration, 2));
        player.addEffect(new MobEffectInstance(MobEffects.MOVEMENT_SPEED,  getDuration() + additiveDuration, 2));
    }
