...
public class BuyTask extends Task {
    ...
    public void getConfig(ConfigGroup config) {
        ...
        config.addItemStack("item", item, v -> item = v, ItemStack.EMPTY, true, false).setNameKey("ftbquests.task.calemicrime.buy");
        config.addLong("count", count, v -> count = v, 1, 1, Long.MAX_VALUE).setNameKey("ftbquests.task.ftbquests.item.count");
    }

    @OnlyIn(Dist.CLIENT)
    public MutableComponent getAltTitle() {
        return new TextComponent("Buy x" + count + " " + item.getHoverName().getString());
    }
